...
相同数据源快照、Extract 层次与自然顺序、Profile 目标和其他明确参与装箱的稳定输入，
必须得到相同的完整规划：TaskBlock 的数量、顺序、边界，以及每块的 Group、Unit 成员
全部一致。

Current、复用、全局去重、非源语、完全保护、Placeholder token、术语命中、临时 ID 和
任务记录都不参与装箱。翻译状态只改变三件事：哪些 Unit 带 ID、无编号语境显示原文还是
目标译文，以及哪些无 ID 块不发送。TaskBlock 无需持久化，不同状态下实际消息也可以不同。

上述顺序规定语义依赖。实现可以提前计算彼此独立的语言、Current 或模型代表等候选状态，
前提是这些状态不成为装箱输入，也不改变完整块。
...
完整块没有任何 ID 时，本轮跳过发送，原块边界和成员保持不变；不把过滤后相邻的块重新
合并，也不移动 Group。每次 Translate 都从权威 Extract 数据重新建立完整规划。
...
任一 Unit 的原文无法完成 Placeholder 或语言投影时，当前 MV/MZ 与 Generic 的整次
Translate 规划失败，不发送任何模型任务，数据库保持不变。诊断定位到失败 Unit，完整
TaskBlock 的语境保留。响应已经返回后的逐 ID 部分成功按各引擎的 Translate 规格处理。
