...
根必须恰好包含 `rule` 数组。定义规则时使用一个或多个 `[[rule]]`；清空项目规则使用：
...
写回方式由已经验证的项目结构唯一决定，因此规则只需选择 Unit 和宽度：
...
WriteBack 先执行配置要求的自动译文标点修复，再按本节断行，最后补续行空白。标点修复
开关与默认值见[正文配置](../runtime/configuration.md#4-writeback-正文开关)。
