...
| 省略参数 | 复用项目当前定义，不重新解析文件 | 不执行 Rules；既有 Rules owner 不变 | 复用项目当前资源 |
...
- 省略 `--rules`：本次根本不执行 Rules，既有 Rules owner 原样保持；
- 提供内容为 `rule = []` 的文件：明确停用并删除 Rules owner 快照。

因此“暂时不传参数”和“传空定义”是两个不同意图，不能互换。
