...
WriteBack 不接受 Lua。它只读取冻结来源、当前提取资产和译文，在项目工作区生成：
...
未译或非 Current Unit 保留冻结原文。Partial 项目允许生成候选，但结果明确报告保留原文
的数量。
...
候选必须重新解析，并证明只发生了受管修改。WriteBack 不执行脚本，也没有发布后回调。

## 3. 一次发布

全部候选完成并验证后，ATT 一次替换整个 `write_back/`。失败时不把候选宣称为成功输出，
上一次成功输出保持。发布结果无法确认时按
[目录发布规格](../runtime/directory-publishing.md)保留现场并报告恢复位置。

`write_back/` 是内容树，不是完整游戏包。操作者负责按实际部署方式放入隔离游戏副本并
验证游戏真正读取了这些文件。
