...
Profile 的消息字符预算和 translation state。切换 locale、切换 `thinking_output`，或修改
本轮实际读取的任一资源，都会使依赖旧 Prompt 的受影响译文不再 Current。关闭时没有读取
`thinking.md`，因此它的变化不会影响该模式的消息、预算或 state。
