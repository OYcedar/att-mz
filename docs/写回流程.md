...
工作区、数据库、冻结源和写回目录的路径全部由共享的
`ProjectWorkspaceLayout` 派生，创建、读取和打开项目不各自解释目录名。
...
flowchart TD
    ...
    RECORD --> WORKSPACE["ProjectWorkspaceLayout<br/>共享受信值"]
    ...
    PUBLISH --> ATOMIC["AtomicDirectoryPublisher<br/>根接口"]
...
`DirectoryStageRequest`，并且只允许发布到当前项目的固定 `write_back_root`。
候选项目身份不匹配、相对路径越界、覆盖重叠均在根调用前失败。

`AtomicDirectoryPublisher` 先通过 `prepare` 在最终目标同级生成完整候选，
再把不可复制的 `StagedDirectory` 交给 `publish(Replace)` 按值消费。
`Replace` 要求固定写回目标当下是现存目录；目标缺失或类型错误时不静默重建。
发布根已接管 token 的终结与清理语义，WriteBack 不在 publish 返回后再调用 `discard`。
已知未发布、已发布但旧备份清理失败、发布结果未知保持不同的结构化语义；
它们都不会自动重试或被 Standard 报告为完全成功。同级暂存、完整递归复制、
目录交换、恢复、清理与符号链接/reparse point 策略都属于该根；生产适配器未实现。
`PublishedButCleanupFailed` 明确表示新输出已可见，错误同时携带未清理的恢复产物路径；
Standard 在 Publisher 错误处停止，不追加完全成功的运行日志。
...
- `FileReader`、`DirectoryLister`、`ExistingDirectoryResolver`、`AtomicDirectoryPublisher`；
- `PersistentEventLog<StandardWriteBackRunLog>`；
- `TrustedLuaRuntimeExecutor`、`SqliteInteractiveSessionFactory`、`LlmRequestExecutor`。

真实 SQLite/文件/Lua VM/日志适配器、生产组合根、真实磁盘端到端写回、封包、
历史修订、冻结篡改检测和行宽修改命令均不在当前完成范围。
