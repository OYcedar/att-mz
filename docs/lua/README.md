...
计算生成、批量变换、诊断和特殊数据库修改。高级 API 使用可读 ID，并执行受管翻译校验；
原始数据库 API 直接执行 SQL，脚本作者自行承担数据与状态一致性。
...
`translations`。查询多条译文时，先合并全部待查 ID，再用一次调用获取完整上下文。
...
这两个高级操作只修改指定位置，不调用模型、修改全局规则或使无关译文失效。`set` 接收
可读 ID 与字符串数组译文，`clear` 只接收可读 ID。
...
raw SQL 可以修改全部项目表、PRAGMA 和事务状态。它不执行 ATT schema 或领域校验，也不
自动备份、修复、开启事务、运行 `foreign_key_check` 或 `quick_check`。关闭外键、删除表或
写入不一致数据，可能使普通 ATT 命令无法再打开项目。执行这类修改前，应确认目标表与数据
关系，并由脚本维护所需约束；需要共同成败的修改放在显式事务中。命令成功只表示脚本执行
结束，数据库是否仍有效由这些修改决定。
...
活动时先回滚该高级操作；`RELEASE`、`ROLLBACK TO` 或 savepoint 清理失败都会使本次脚本
停止，即使 `pcall` 捕获错误也不能继续提交外层事务。运行根随后回滚仍活动的根事务；根
回滚失败按结果未知报告。最外层 `RELEASE` 报错且 SQLite 已经结束事务时，ATT 无法从返回码
确定已经提交还是自动回滚，同样报告结果未知。没有这类高级 API 清理失败时，raw SQL 与
脚本显式事务继续遵守上面的提交、回滚规则。
...
ATT schema 校验，因此仍可用于调查或继续修改当前 `project.db`。
