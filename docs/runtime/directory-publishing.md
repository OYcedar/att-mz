...
Windows 等价名称使用 ordinal ignore-case 的 UTF-16 code unit 身份，不执行 Unicode
case folding、规范化或兼容折叠。例如 `K` 与 `k` 等价，Kelvin 符号 `K` 与二者均不
等价；补充平面的 surrogate 单元保持原样。存储、运行时和输出文档领域边界共同消费
这一份身份规则。
...
ATT 只把每个逻辑路径独占拥有一个物理文件的目录树当作可冻结来源或可发布候选。任何
已经打开的普通文件只要链接数不等于 1 就显式失败，无论其他别名位于当前树内还是树外。
树外别名可以绕过当前目录的路径观测修改同一对象；树内别名则让多个逻辑路径共享一次
物理修改。二者都会破坏指纹、候选隔离和单次目录发布共同依赖的“一个路径、一个对象”
不变量。

该规则同样适用于只读来源扫描：扫描结果承诺的是一份自包含、能在后续候选构建中继续
使用的冻结快照，而不只是某一瞬间能够读出字节。硬链接拒绝是现行快照与发布语义，不是
文件数量、字节数或资源保护上限。
...
每个锁文件使用上述稳定 Windows ordinal 非大小写身份摘要。实际取得锁时验证普通目录、
reparse 和系统锁能力；不对整个 `projects.root` 做无关的文件系统品牌预检。
