...
目录发布器把完整候选安全替换为项目或 WriteBack 输出，并在中断后按 journal 恢复同一目标。
本规格同时定义来源读取、候选文件树、目标锁和恢复现场的边界。

## 1. 安全文件树

每个来源和候选使用可独立识别的普通文件。reparse point、硬链接、Windows 大小写等价冲突、
路径逃逸或读取期间对象身份变化会使操作失败。文件字节、目录项、深度、树总字节和恢复产物
数量由实际环境承载，ATT 不另设总量上限。
...
发布目标使用经过校验的绝对路径，候选内目标使用安全相对路径。来源、候选和目标物理隔离。

项目与发布目标的父目录必须位于**本地固定磁盘的 NTFS 卷**，并使用大小写不敏感的目录名称语义。
ATT 在固定父目录和准备目标时校验这些条件；网络盘、可移动介质、其他文件系统和启用大小写
敏感的目录会被拒绝。将发行根及项目放在满足条件的可写目录；目录交换中的 stage、backup 与
目标位于同一卷。
