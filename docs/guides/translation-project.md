...
Survey 生成一份按关系组压缩、默认 `unresolved` 的所有权决定模板。Agent 只修改已经获得证据的
组，确需拆组时才改成少量成员决定；不得在聊天消息、补丁或临时脚本中手写全量决定文件。

插件参数的序列化层级同样来自消费者证据，不从值的 JSON 外形猜测。活动插件源码中当前插件参数
对象的精确参数直接进入 `JSON.parse`，只证明该参数顶层的一次解码；schema 中某一层明确声明
`struct` / array 容器，只证明对应层。Survey 逐层沿这些事实枚举字符串并记录解码位置，不能把顶层
证据递归套用到容器内任意 JSON 外形字符串；否则保留当前层字符串等待进一步调查。已证明的
序列化参数无法解码时，它是结构审核项，不得把整段损坏 JSON 当成可翻译正文。
...
同一容器中的大量独立记录必须按真实记录边界建立多个 Group，让 ATT 在 Group 之间组合任务；
不得把整个数组、补丁日志或物理文件直接做成一个超长 Group。
