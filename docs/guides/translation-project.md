...
模型、用 Lua 精确修订译文和生成 WriteBack 输出，除非用户明确排除，不得为每条命令再次
询问。改 Endpoint、Model、parameters 或凭据，覆盖游戏原件，写入外部系统，或执行任务
没有要求的破坏性操作，仍需用户提供明确新值或授权。
...
1. 确认实际 `att.exe`、版本、SHA-256、发行目录和调用 cwd；
...
| 已经定位的单个或批量译文需要人工或 agent 直接修订 | 读取 [Lua](../lua/README.md)，再按验收指南复查 |
| 候选内容、布局、外部转换、部署或实际加载错误 | WriteBack 或外部消费步骤 |

Lua 不是只处理“相同原文要不同译文”的特例。它也是 ATT 当前提供的原子人工或 agent
修订入口，但必须先按 Lua 规格取得当前 Unit 的精确身份和完整语境，并在修订后重新验收。
