...
ATT 的 Windows x64 `att.exe` 把所需的 Visual C++ Runtime 可再发行代码静态链接进程序。
随包 Formic v0.1.0 需要 `VCRUNTIME140.dll`，因此该 DLL 与 `formic.exe` 放在同一目录。
相关代码版权归 Microsoft Corporation 所有。
