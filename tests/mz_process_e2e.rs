...
fn init_extract_translate_and_write_back_cross_process_with_real_roots() {
    ...
    init_arguments.extend(arguments(&[
        "--source-language",
        "ja",
        "--target-language",
        "zh-Hans",
    ...
    let init = run_att(root, init_arguments);
...
assert(ctx.db.execute(
  "INSERT INTO lua_process_probe (phase, detail) VALUES (?1, ?2)",
...
fn assert_lua_probes(database: &Path, expected_phases: &[&str]) {
    ...
    if expected_phases.contains("write_back") {
        ...
        assert!(output.join("data/Items.json").is_file());
    }
}
            ...
            let result = (|| {
                ...
                for request_index in 0..2 {
                    let request = loop {
                        match self.listener.accept() {
                            Ok((stream, _)) => {
                                break serve_chat_completion(stream, request_index)?;
                            }
...
fn serve_chat_completion(
    ...
    let (request_id, response_id, content, prompt_tokens, completion_tokens, total_tokens) =
        match request_index {
            ...
            _ => return Err("本地 LLM 收到超出契约的请求".to_owned()),
        };
    let body = json!({
        "id": response_id,
        "choices": [{
