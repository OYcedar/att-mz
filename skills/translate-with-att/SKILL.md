...
避免返工而阻塞首次可用版本。每轮结束都区分：当前游戏的修订、可复用的同类游戏方法，以及
需要修复的 ATT/工具问题。

开始调查前，按当前游戏的结构读取[同类游戏翻译方法](references/game-type-methods.md)中最接近
的一节；只有适用条件成立时才采用。真实翻译和用户反馈证明了新的通用方法时，更新该专栏，
不得写入游戏名、具体文本或只能解释单个项目的路径。
...
- 首次交付完成只表示用户已有可测试版本；用户实玩反馈尚未处理时，任务处于等待反馈状态，
  不能声称循环结束。反馈到达后只复查受影响范围，并把确认可复用的方法写回类型专栏。
