...
- 固定术语责任 `TRN-004` 已存在；全部玩家可见文本载体已经进入全局地图，漏收载体先返回
  提取，不以当前已提取集合冒充完整语料。
...
在正式批量 Translate 请求前，按[术语现行规格与制作指南](../../../docs/rpg-maker/terminology.md)
完成并记录：

1. 以现行 Builtin 覆盖矩阵、MV/MZ 特殊字段及其真实上下文建立候选种子，包括角色、称号、
   职业、技能、物品、装备、敌人、状态、地点、System 分类和引擎特有 Speaker/姓名投影；
   特殊字段只提供高价值入口，不自动成为术语。
2. 对任务范围内全部玩家可见文本及运行时消费者做全局取证，覆盖 Builtin、Rules、
   Lua Standard、Managed、低级 Lua 私有文本和已启用插件文本。代码、路径、ID、协议壳及
   非玩家字符串不因存在于文件中而成为候选。
3. 逐条确认概念边界、目标译名、稳定异体或简称以及 `triggers`。不得按频次机械收录，
   不得把完整结构化字段、一次性句子或全部提取单位复制为术语。
4. 在完整玩家文本集合上验证正命中、漏命中、同形误命中、互相包含、歧义上下文和
   Placeholder opaque 边界；需要依赖条目顺序解决冲突的候选不得收录。
5. 在账本证据中记录候选来源、跨载体实际用例、选择或排除理由、译名依据、trigger
   正反例和最终文件定位。无可用术语同样完成本责任，以证据证明并提供权威
   `term = []`，不得用省略 `--terms` 假装已经核实。

术语基线建立后发生新载体、新语义、译名或 trigger 变化时，按配置变更门追加 `CHG-*`，
重开 `TRN-004` 的后继 TODO 和受影响的翻译、写回、封包验收。
...
- `TRN-004` 为 `DONE`，术语、Placeholder、语言和关键样本质量已验证。
