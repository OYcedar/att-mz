...
任务包返回内容只等同于工作子代理候选。主代理回收后，仍必须按 `agent-review-workflow.md` 写入工作报告或等效记录，派发审查子代理，关闭 blocker，写阶段裁决，再运行 validate/import。
...
```powershell
<A.T.T MZ 可执行文件> prepare-agent-workspace --game <游戏标题> --output-dir <工作区>
```

用户可选择当前会话完成、外部协作任务包或混合处理。默认使用当前会话完成并自动推进；只有用户表示额度/时间有限、希望把任务带走处理，或当前会话无法承载候选分析时，才询问是否导出外部协作任务包。多项候选分析会消耗较多上下文和模型额度；额度有限时，建议主代理把任务包文件夹交给用户带走处理。
...
任务包只能覆盖五个术语候选分组、插件规则、事件指令规则和 Note 标签规则。它们只产出工作候选，不能替代审查子代理或主代理裁决。占位符规则最终生成、覆盖扫描和导入，最终术语表合并与正文术语表维护，正文翻译、重置译文、写进游戏文件和字体覆盖，都不能导出为普通任务包。
...
9. 派发对应审查子代理，读取审查报告并关闭 blocker。
10. 写入对应 `review-decisions/<阶段ID>.json`。
11. 对规则类结果运行对应 `validate-*`；通过后才运行对应 `import-*`。
12. 小范围格式问题可由主代理修正；大面积错误必须要求重做或改由主代理完成。

任务包答案通过验收、审查和主代理裁决前，不能导入规则，不能启动正文翻译，不能写进游戏文件。
