...
本阶段采用审查型工作流。分组术语候选由工作子代理产出，术语审查子代理检查原文照抄、中英混杂、源文残留、机械拼接、同名冲突和上下文一致性；主代理合并最终字段译名表和正文术语表后，还必须经过整体术语审查和阶段裁决，才能运行 `import-terminology`。
...
第一轮只处理术语候选。工作子代理只能读取自己的源文件和上下文，只写对应候选文件、`agent-reports/terminology/<术语分组>.json` 和 `<工作区>/agent-scratch/terminology/<术语分组>/` 下的审计材料：
...
主动发现：规模较大的分组必须写一次性脚本或统计产物，检查空值、source == translation、中英混杂、重复源文不同译名、明显机械拼接、疑似保留源文项和同类命名风格。
交叉验证：至少用源文件条目和一个上下文来源互相核对；存在说话人上下文或数据库上下文时，必须说明关键译名如何与上下文一致。
完成报告：写入 <工作区>/agent-reports/terminology/<术语分组>.json，说明总条数、空值数、原文照抄数、中英混杂候选、读取的上下文、脚本和统计产物、关键统一译名、疑难项、自检结果、保留源文理由和交叉验证摘要。
```
...
- `terminology_group_review`：审查各分组候选。
- `terminology_bundle_review`：审查最终 `terminology/field-terms.json` 和 `terminology/glossary.json`。

审查子代理可以在 `<工作区>/agent-scratch/terminology/<审查任务ID>/` 下写一次性脚本，复核候选和最终表。审查报告写入 `<工作区>/review-reports/terminology/<审查任务ID>.json`， findings 必须使用 `blocker`、`warning` 或 `info` 分级。

术语审查必须覆盖：

- `source_text == translated_text` 数量和比例，按类别统计。
- 空值。
- 中英混杂、源文残留和机械拼接。
- 重复源文不同译名、同名跨类别冲突和上下文不一致。
- 字段译名表和正文术语表同名条目的译名一致性。
- 正文术语表是否太小，漏掉核心角色、地名、组织、关键道具、关键技能或系统概念。
- 正文术语表是否太大，变成字段译名表镜像。
- 字段包装形式、整句、一次性枚举、调试项、定位信息或说明字段是否误入正文术语表。
- 原文保留是否有理由。

如果某分组原文照抄比例异常高、正文术语表明显漏核心概念、字段包装误入正文术语表或同名冲突，审查报告必须给 `blocker`，除非工作报告给出充分保留理由并经主代理裁决确认。
...
- 派发 `terminology_bundle_review` 审查最终字段译名表和正文术语表。
- 写入 `<工作区>/review-decisions/terminology.json`；存在未关闭 `blocker` 时禁止导入。
...
```powershell
<A.T.T MZ 可执行文件> import-terminology --game <游戏标题> --input <工作区>/terminology/field-terms.json --glossary-input <工作区>/terminology/glossary.json
```

字段译名表 key 不能改，正文术语表只能包含 `terms`。导入校验会检查字段译名表完整性、空译名、正文术语表空值、字段译名表内部同名冲突，以及正文术语表和字段译名表同名条目的译名一致性；它不会要求字段译名表的每个原文都原样进入正文术语表。任一术语候选质量明显不合格，或整体术语审查存在未关闭 blocker 时，退回对应候选子代理重做、重新审查或由主代理亲自重译对应字段，不能把坏候选保存为当前游戏术语表。
