...
- 运行期下一步优先以 `doctor --game <游戏标题> --no-check-llm` 的 `flow_decision`、`flow_reason` 和 `flow_next_command` 为准；主代理负责执行或解释裁决，不把已能自动判断的问题丢给用户。
...
- `<发行版目录>` 是 A.T.T MZ 发行包目录。默认命令是 `<A.T.T MZ 可执行文件> <命令> ...`。
