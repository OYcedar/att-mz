...
`scale-metrics.csv` 由 Formic 源码工程中的规模实验生成，随包程序的正常作业不会创建它。需要调查吞吐与内存曲线时，先按[来源记录](../FORMIC-SOURCE.md)取得独立 Formic 源码，再在该工程根目录执行：
...
该文件由源码工程忽略，正常运行、续跑和结果发布均不读取它。实验结束后可删除，下次实验会重新生成。
