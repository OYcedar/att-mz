...
只输出一个 JSON object，可以直接输出，也可以放在唯一一个闭合的 `json` Markdown 代码块中；使用代码块时，块外不要添加任何内容。不要输出其他 JSON、代码块或前后说明。根对象直接以各个 `id` 为 key。每个 value 必须且只能包含 `source` 和 `translation`：`source` 原样回显该 `id` 输入的 `text` 字符串数组，`translation` 是译文字符串数组。
