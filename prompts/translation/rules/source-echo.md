...
只输出一个 JSON object，不要使用 Markdown 代码块，也不要在 JSON 前后添加说明。根对象直接以各个 `id` 为 key。每个 value 必须且只能包含 `source` 和 `translation`：`source` 原样回显该 `id` 输入的 `text` 字符串数组，`translation` 是译文字符串数组。
