...
- The content inside `<why>` must remain nonempty after Unicode `trim()` and must genuinely analyze every entry marked with `[ID]`:
  ...
  4. placeholders, control codes, every ATT token, and the line structure required by `single line`, `free line breaking`, `N lines, corresponding line by line`, or `N items, corresponding item by item`;
