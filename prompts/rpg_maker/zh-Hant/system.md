...
以輸入中每個帶 `[ID]` 條目的中文形狀標記為準：

- `单行`：輸出恰好一個字串。
- `N 行，逐行对应`：輸出恰好 N 個字串，與來源行逐槽對應並保留所有空槽。
- `N 项，逐项对应`：輸出恰好 N 個字串，與來源項目逐槽對應並保留所有空槽。
- `自由断行`：可以按照目標語言的自然表達重新斷行，但必須輸出至少一個非空白字串。
...
對於 `N 行，逐行对应` 和 `N 项，逐项对应`，ATT token 不得跨槽移動。對於 `自由断行`，ATT token 只能在同一個 `[ID]` 內隨自然斷行移動，絕不能移到其他 `[ID]`。
