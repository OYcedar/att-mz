//! MZ note 与事件注释共同使用的简单标签扫描。
//!
//! Rules Extraction 和 WriteBack 必须对 `<name:value>` 的 occurrence 使用同一套
//! 字节边界，否则数据库中的结构化位置无法稳定指回冻结原文。
...
///
/// 该语法刻意保持 Rules 已确认的小语义：使用第一个冒号分隔名称和值；没有冒号、
