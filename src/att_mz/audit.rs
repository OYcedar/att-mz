...
//! 追加、刷盘、轮转与恢复完整记录，不理解 MZ 项目、命令或位置。
...
use super::text::{MzLocation, MzLocationStep, MzSource};
use super::translate::standard::{
...
use super::write_back::StandardWriteBackSummary;
use super::write_back::standard::{
...
impl AuditContext {
    pub(crate) fn init(run_id: RunId, project: impl Into<String>) -> Self {
        Self::new(run_id, project, AuditCommandContext::Init)
    }

    pub(crate) fn extract(run_id: RunId, project: impl Into<String>) -> Self {
        Self::new(run_id, project, AuditCommandContext::Extract)
    }
    ...
    pub(crate) fn write_back(run_id: RunId, project: impl Into<String>) -> Self {
        Self::new(run_id, project, AuditCommandContext::WriteBack)
    }

    fn new(run_id: RunId, project: impl Into<String>, command: AuditCommandContext) -> Self {
        Self {
            run_id,
...
impl TranslationTaskWire {
    fn from_record(record: &TranslationTaskLogRecord) -> Self {
        Self {
            ...
            confirmed_written_locations: None,
            accepted: record
                .accepted()
                .iter()
    ...
    fn from_confirmed_record(record: &TranslationTaskLogRecord) -> Self {
        ...
        wire.confirmed_written_locations = Some(
            wire.accepted
                .iter()
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if let Some(task) = task {
            ...
            let expected_written = task
                ...
                .sum();
            if committed {
                if task.confirmed_written_locations != Some(expected_written) {
                    return Err(
                        "completed 的 confirmed_written_locations 必须等于结构化写入位置数"
                            .to_owned(),
                    );
                }
            } else if task.confirmed_written_locations.is_some() {
                return Err("未提交任务的 confirmed_written_locations 必须为 null".to_owned());
            }
        }
        Ok(())
    }
...
mod tests {
    ...
    fn context_only_allows_profile_on_translate() {
        assert_eq!(AuditContext::init(run_id(), "demo").profile(), None);
        assert_eq!(
            AuditContext::translate(run_id(), "demo", "quality").profile(),
            Some("quality")
        );
        assert_eq!(AuditContext::write_back(run_id(), "demo").profile(), None);
    }
    ...
    async fn task_intent_and_terminal_share_operation_but_not_event_identity() {
        ...
        let (ledger, finalizer) =
            JsonLinesAuditLedger::start(directory.path().to_path_buf(), config())
                .expect("审计账本应可启动");
        let run = ledger.bind(AuditContext::translate(run_id(), "demo", "quality"));
    ...
    async fn command_mismatch_is_rejected_before_any_record_is_enqueued() {
        ...
        let (ledger, finalizer) =
            JsonLinesAuditLedger::start(directory.path().to_path_buf(), config())
                .expect("审计账本应可启动");
        let run = ledger.bind(AuditContext::init(run_id(), "demo"));
        let operation_id = run.new_operation_id().expect("操作身份应可生成");
    ...
    async fn write_back_terminal_preserves_the_exact_publish_state() {
        ...
        let (ledger, finalizer) =
            JsonLinesAuditLedger::start(directory.path().to_path_buf(), config())
                .expect("审计账本应可启动");
        let run = ledger.bind(AuditContext::write_back(run_id(), "demo"));
        let operation_id = run.new_operation_id().expect("操作身份应可生成");
