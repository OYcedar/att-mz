...
pub(crate) struct TranslateService<B> {
    execution_builder: B,
    cancellation: CooperativeCancellation,
}

impl<B> TranslateService<B> {
    pub(crate) fn new(execution_builder: B, cancellation: CooperativeCancellation) -> Self {
        Self {
            execution_builder,
            cancellation,
        }
    ...
    B: SelectedTranslationExecutionBuilder,
{
    pub(crate) async fn execute(
        ...
        TranslateServiceError<B::Error, <B::Translation as RpgMakerTranslation>::Error>,
    > {
...
/// 翻译用例在直接依赖边界上遇到的阶段失败。
#[derive(Debug)]
pub(crate) enum TranslateServiceError<BE, TE> {
    BuildExecution(BE),
    Translation { source: TE },
}

impl<BE, TE> fmt::Display for TranslateServiceError<BE, TE>
where
    BE: Error,
    TE: Error,
{
...
impl<BE, TE> Error for TranslateServiceError<BE, TE>
where
    BE: Error + 'static,
    TE: Error + 'static,
{
...
mod tests {
    ...
    async fn typed_builder_cancellation_is_a_normal_cancelled_result() {
        ...
        let service = TranslateService::new(TypedCancellingBuilder, cancellation);
