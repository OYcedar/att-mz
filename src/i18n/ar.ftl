...
error-rpg-maker-prompt-unavailable = تعذر استخدام موجه نظام RPG Maker للزوج اللغوي { $source } → { $target } في { $path }، لذلك لم تبدأ الترجمة. تأكد من وجود الملف، ومن تطابق اسمه تمامًا مع الزوج اللغوي، ومن أن محتواه UTF-8 غير فارغ.
