...
error-rpg-maker-prompt-unavailable = Le prompt système RPG Maker pour { $source } → { $target }, situé à { $path }, ne peut pas être utilisé ; la traduction n’a donc pas commencé. Vérifiez que le fichier existe, que son nom correspond exactement à la paire de langues et que son contenu UTF-8 n’est pas vide.
