...
error-rpg-maker-prompt-unavailable = Не удалось использовать системный промпт RPG Maker для { $source } → { $target } из файла { $path }, поэтому перевод не начат. Убедитесь, что файл существует, его имя в точности соответствует языковой паре, а содержимое имеет формат UTF-8 и не является пустым.
