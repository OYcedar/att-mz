...
cli-ownership-export-about = Exportar la propiedad del texto de cada unidad RPG Maker extraída
cli-translation-export-about = Exportar el texto original, la traducción actual y el estado de cada unidad extraída
...
cli-jsonl-file-help = Archivo de exportación JSONL
cli-retry-rejected-help = Reintentar los candidatos Rejected guardados
cli-manual-selection-help = Selección para exportar: pending (predeterminada), rejected o all
cli-manual-ids-help = Exportar las entradas con los ID naturales de este archivo JSONL
cli-layout-rules-help = Cargar y guardar las reglas de formato WriteBack desde un archivo TOML
...
log-phase-name = { $phase ->
    [check_project] Comprobación del proyecto
    [scan_source] Análisis de los archivos fuente
    [prepare_candidate] Preparación del candidato
    [update_database] Actualización de la base de datos
    [publish] Publicación
    [builtin] Extracción Builtin
    [builtin_documents] Análisis de documentos Builtin
    [builtin_work_units] Extracción de unidades de texto Builtin
    [builtin_commit] Confirmación en la base de datos de Builtin
    [rules] Extracción Rules
    [rules_documents] Análisis de documentos Rules
    [rules_matches] Búsqueda de coincidencias Rules
    [rules_commit] Confirmación en la base de datos de Rules
    [lua] Ejecución de Lua
    [planning] Planificación de tareas de traducción
    [confirmed_tasks] Confirmación de tareas de traducción
    [read_assets] Lectura del contenido del proyecto
    [plan_rpg_maker_write_back] Planificación de WriteBack
    [rewrite_documents] Reescritura de documentos
    [validate_candidate] Validación del candidato
   *[other] __ATT_FALLBACK__
}
...
diagnostic-failure-value = { $code ->
    ...
    [response_id_invalid] Un elemento de respuesta tiene un output ID no válido
    [response_id_unexpected] La respuesta contiene un output ID que no se solicitó
    [response_id_duplicate] La respuesta contiene el mismo output ID más de una vez
    [response_id_missing] Falta un output ID solicitado en la respuesta
    [response_translation_not_array] translation debe ser una matriz de cadenas
    [response_translation_item_not_string] Un elemento de la matriz translation no es una cadena
    [response_echo_shape_invalid] El objeto source repetido no coincide con la estructura source/translation solicitada
    [response_echo_source_item_not_string] Un elemento de la matriz source repetida no es una cadena
    [response_translation_blank] La traducción devuelta está vacía
    [response_translation_text_invalid] La traducción devuelta contiene un salto de línea, NUL o marca de orden de bytes no permitido
    [response_placeholder_snapshot_invalid] La instantánea de Placeholder usada para validar la respuesta no es válida
    [response_placeholder_identity_or_count_mismatch] La traducción cambió la identidad o la cantidad de los Placeholders obligatorios
    [response_placeholder_missing] Falta un token de control obligatorio en la traducción
    [response_placeholder_unexpected] La traducción contiene un token de control inesperado
    [response_placeholder_order_mismatch] La traducción cambió el orden obligatorio de los tokens de control
    [response_placeholder_binding_mismatch] La traducción cambió cómo se vinculan al texto los Placeholders obligatorios
    [response_placeholder_boundary_mismatch] La traducción añadió o eliminó un límite de Placeholder obligatorio
    [response_placeholder_reserved_token] La traducción contiene un token Placeholder reservado
    [response_placeholder_ambiguous] Un Placeholder devuelto no puede asociarse inequívocamente con un token obligatorio
    [response_control_token_invalid] La estructura de tokens de control devuelta no es válida
    [response_text_segment_count_mismatch] La respuesta cambió la cantidad obligatoria de segmentos de texto
    [response_text_segment_shape_mismatch] La respuesta cambió la estructura obligatoria de los segmentos de texto
    [response_line_count_mismatch] La matriz translation tiene una cantidad incorrecta de elementos
    ...
    [response_all_outputs_rejected] Se rechazaron todas las salidas de la respuesta Assistant
...
diagnostic-input-field = Campo: { $field }
diagnostic-input-failure = { $code ->
    [syntax] Sintaxis TOML no válida
    [missing_field] Falta un campo obligatorio
    [unknown_field] El formato actual no admite este campo
    [duplicate_field] El campo está duplicado
    [type_mismatch] El campo tiene un tipo incorrecto
    [invalid_value] El valor del campo no es válido
   *[other] __ATT_FALLBACK__
}
diagnostic-expected-type = Tipo esperado: { $expected ->
    [string] cadena
    [integer] entero
    [boolean] booleano
    [string_or_boolean] cadena o booleano
    [string_array] matriz de cadenas
    [integer_array] matriz de enteros
    [table] tabla
    [table_array] matriz de tablas
    [array] matriz
    [object] objeto
   *[other] __ATT_FALLBACK__
}
diagnostic-response-item = elemento de respuesta { $item }
...
manual-value = { $code ->
    [translation_byte_order_mark] El elemento { $line } de translation contiene un BOM (U+FEFF)
    [remove_byte_order_mark] Eliminar el carácter BOM (U+FEFF) de la traducción
    [keep_placeholders] Restaurar los Placeholders del original en la traducción, conservando su cantidad, el orden requerido y sus segmentos de texto
...
task-record-final-status = Estado: { $state ->
    ...
    [unavailable_rejected_committed] No disponible; candidatos rechazados guardados
